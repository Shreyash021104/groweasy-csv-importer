...
5. SKIP RULE: if a row contains NEITHER a usable email address NOR a usable phone/mobile number anywhere in its data, you MUST set status to "skipped" with a short human-readable skip_reason (e.g. "No email or phone number found"), and omit "record".
6. Otherwise set status to "imported" and populate "record" with as many fields as you can confidently determine, using "" for any field you cannot determine. Never fabricate data that isn't present or implied in the row.
